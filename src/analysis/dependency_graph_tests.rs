...
mod tests {
    ...
    fn test_component_weights() {
        ...
        // Add components
        graph.add_component("comp1".to_string(), vec![]);
        graph.add_component("comp2".to_string(), vec![]);

        // Test default uniform weights
        let weight1 = graph.get_component_weight(&"comp1".to_string());
        assert_eq!(weight1.computational_cost, 1.0);
        assert_eq!(weight1.memory_usage, 0);

        // Test custom weight setting
        let custom_weight = ComponentWeight::new(5.0, 1024);
        graph.set_component_weight(&"comp1".to_string(), custom_weight);

        let updated_weight = graph.get_component_weight(&"comp1".to_string());
        assert_eq!(updated_weight.computational_cost, 5.0);
        assert_eq!(updated_weight.memory_usage, 1024);

        // Test weight from component type
        graph.set_component_weight_from_type(&"comp2".to_string(), "processor");
        let processor_weight = graph.get_component_weight(&"comp2".to_string());
        assert_eq!(processor_weight.computational_cost, 10.0);
        assert_eq!(processor_weight.memory_usage, 1024);
    }

    #[test]
    fn test_component_weight_types() {
        use crate::analysis::dependency_graph::ComponentWeight;

        // Test predefined component types
        let processor_weight = ComponentWeight::from_component_type("processor");
        assert_eq!(processor_weight.computational_cost, 10.0);

        let memory_weight = ComponentWeight::from_component_type("memory");
        assert_eq!(memory_weight.computational_cost, 5.0);

        let router_weight = ComponentWeight::from_component_type("router");
        assert_eq!(router_weight.computational_cost, 3.0);

        let counter_weight = ComponentWeight::from_component_type("counter");
        assert_eq!(counter_weight.computational_cost, 1.0);

        // Test unknown type defaults to uniform
        let unknown_weight = ComponentWeight::from_component_type("unknown");
        assert_eq!(unknown_weight.computational_cost, 1.0);
        assert_eq!(unknown_weight.memory_usage, 0);
    }
}
