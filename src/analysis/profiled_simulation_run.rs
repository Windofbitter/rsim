use crate::analysis::{
    ComponentPartition, DependencyGraph, GraphPartitioner, PartitionerFactory, ProfilingReport,
};
...
impl ProfiledSimulationRun {
        ...
        F: Fn() -> Vec<Box<dyn BaseComponent>>,
    {
        ...
        println!(
            "Built initial dependency graph: {} components, {} edges",
...
impl ProfiledSimulationResult {
    ...
    pub fn print_summary(&self) {
        ...
        println!(
            "Weighted graph: {} components, {} edges",
        ...
        if let Some(ref partition) = self.partition {
            ...
            println!("Modularity: {:.3}", partition.quality_metrics().modularity);
            println!(
                "Overall quality score: {:.3}",
                partition.quality_metrics().overall_score
            );
        } else {
            println!("\n=== Partition Status ===");
    ...
    pub fn partition_components(
        ...
        println!(
            "Partitioned {} components into {} threads using {} algorithm",
            partition.num_components(),
            partition.num_threads(),
            algorithm
        );
        println!(
            "Partition quality - Cut weight: {}, Modularity: {:.3}",
            partition.quality_metrics().cut_weight,
            partition.quality_metrics().modularity
        );
    ...
    pub fn partition_components_with_partitioner(
        &mut self,
