...
impl ConnectionManager {
    ...
    pub fn connect(
        ...
        if !self.processing_components.contains_key(&source.0)
            && !self.memory_components.contains_key(&source.0)
        {
            return Err(format!("Source component {} does not exist", source.0));
        }
        if !self.processing_components.contains_key(&target.0)
            && !self.memory_components.contains_key(&target.0)
        {
    ...
    pub fn connect_memory(
        ...
        if self
            .memory_connections
            .contains_key(&(proc_id.clone(), port.clone()))
        {
            return Err(format!(
                "Port ({}, {}) already connected to memory",
                proc_id, port
            ));
        }
    ...
    pub fn add_probe(
        ...
        if !self.processing_components.contains_key(&source_port.0)
            && !self.memory_components.contains_key(&source_port.0)
        {
