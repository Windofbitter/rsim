...
impl MetricsCollector {
    ...
    fn handle_order_fulfilled(&mut self, event: &dyn Event) {
        ...
        self.fulfilled_orders.push((
            order_id.clone(),
            timing.placement_cycle,
            self.current_cycle,
        ));
...
impl BaseComponent for MetricsCollector {
    ...
    fn react_atomic(&mut self, events: Vec<Box<dyn Event>>) -> Vec<(Box<dyn Event>, u64)> {
        ...
        for event in events {
            log::debug!("[MetricsCollector {}] Processing event type: {}", self.id, event.event_type());
            match event.event_type() {
                ...
                "OrderFulfilledEvent" => {
                    log::info!("[MetricsCollector {}] Received OrderFulfilledEvent", self.id);
