...
use super::super::events::{BreadReadyEvent, TriggerProductionEvent};
...
impl Baker {
    ...
    fn handle_trigger_production(&mut self) -> Option<BreadReadyEvent> {
        ...
        if self.production_mode == ProductionMode::OrderBased {
            if let Some(target) = self.target_quantity {
                if self.completed_count >= target {
                    log::info!(
                        "[Baker {}] Order complete. Produced {} items.",
                        self.id,
        ...
        Some(BreadReadyEvent::new(self.id.clone(), None, item_id))
    }
}
            ...
            match event.event_type() {
                "TriggerProductionEvent" => {
                    ...
                    if let Some(held_item) = self.held_item.take() {
                        log::info!(
                            "[Baker {}] Retrying held item after space became available.",
                            self.id
                        );
                ...
                "ItemDroppedEvent" => {
                        ...
                        && self.completed_count > 0
                    {
                        self.completed_count -= 1;
                        log::info!(
                            "[Baker {}] Adjusted completed count to {} after drop",
                            self.id,
                            self.completed_count
                        );
                    }
