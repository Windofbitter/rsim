...
use super::super::events::{MeatReadyEvent, TriggerProductionEvent};
...
impl Fryer {
    ...
    fn handle_trigger_production(&mut self) -> Option<MeatReadyEvent> {
        ...
        if self.production_mode == ProductionMode::OrderBased {
            if let Some(target) = self.target_quantity {
                if self.completed_count >= target {
                    log::info!(
                        "[Fryer {}] Order complete. Produced {} items.",
                        self.id,
        ...
        Some(MeatReadyEvent::new(self.id.clone(), None, item_id))
    }
}
...
impl BaseComponent for Fryer {
    ...
    fn emitted_events(&self) -> &[&'static str] {
        &["MeatReadyEvent", "TriggerProductionEvent"]
    }
            ...
            match event.event_type() {
                "TriggerProductionEvent" => {
                    ...
                    if let Some(held_item) = self.held_item.take() {
                        log::info!(
                            "[Fryer {}] Retrying held item after space became available.",
                            self.id
                        );
                ...
                "ItemDroppedEvent" => {
                        ...
                        && self.completed_count > 0
                    {
                        self.completed_count -= 1;
                        log::info!(
                            "[Fryer {}] Adjusted completed count to {} after drop",
                            self.id,
                            self.completed_count
                        );
                    }
